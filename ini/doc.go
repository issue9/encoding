...
//
// ini还提供了Unmarshal与Marshal两个函数，功能类似于xml中的同名函数。
// 也可以通过struct tag指定名称，但格式上稍微有一点不一样，其格式如下：
//  type User struct {
//      ID int `ini:"name:id"`
//  }
// 通过struct tag还可能指定一个转换函数，指定数据之间是是如何相互转换的。
// 比如上图，如何从一个字符串的id转换成int类型的User.ID：
//  type User struct {
